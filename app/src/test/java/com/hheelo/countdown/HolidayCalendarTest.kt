...
class HolidayCalendarTest {
    ...
    fun availableYearsIncludesAllBundledHolidayData() {
        val years = HolidayCalendar.availableYears()
    ...
    fun lookupReportsWhenCurrentYearIsExhaustedAndNextYearIsMissing() {
        val result = HolidayCalendar.lookup(LocalDate.of(2028, 10, 9))
    ...
    fun lookupReportsMissingCurrentYearWhenNoCheckedYearHasData() {
        val result = HolidayCalendar.lookup(LocalDate.of(2029, 1, 1))
    ...
    fun lookup2025NewYear() {
        val result = HolidayCalendar.lookup(LocalDate.of(2025, 1, 1))
    ...
    fun lookup2025SpringFestival() {
        val result = HolidayCalendar.lookup(LocalDate.of(2025, 1, 20))
    ...
    fun lookup2025NationalDayMergedWithMidAutumn() {
        val result = HolidayCalendar.lookup(LocalDate.of(2025, 9, 30))

        assertEquals(HolidayLookupStatus.UPCOMING_FOUND, result.status)
        assertEquals("国庆+中秋", result.upcomingHoliday?.name)
    ...
    fun lookup2026SpringFestivalDuringHoliday() {
        val result = HolidayCalendar.lookup(LocalDate.of(2026, 2, 20))
    ...
    fun lookup2027SpringFestival() {
        val result = HolidayCalendar.lookup(LocalDate.of(2027, 1, 15))
    ...
    fun lookup2027NationalDay() {
        val result = HolidayCalendar.lookup(LocalDate.of(2027, 9, 28))

        assertEquals(HolidayLookupStatus.UPCOMING_FOUND, result.status)
        assertEquals("国庆", result.upcomingHoliday?.name)
    ...
    fun lookup2028SpringFestival() {
        val result = HolidayCalendar.lookup(LocalDate.of(2028, 1, 10))
    ...
    fun lookup2028NationalDayMergedWithMidAutumn() {
        val result = HolidayCalendar.lookup(LocalDate.of(2028, 10, 3))
    ...
    fun lookupCrossesYearBoundaryFrom2027To2028() {
        val result = HolidayCalendar.lookup(LocalDate.of(2027, 10, 8))
    ...
    fun holidayCountPerYear() {
        assertEquals(6, HolidayCalendar.holidayRanges(2025).size)
        assertEquals(7, HolidayCalendar.holidayRanges(2026).size)
        assertEquals(7, HolidayCalendar.holidayRanges(2027).size)
        assertEquals(6, HolidayCalendar.holidayRanges(2028).size)
    }
}
